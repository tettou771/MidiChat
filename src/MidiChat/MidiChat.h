...
シーケンスは、a/b beat だとして、n は拍数ではなく小節の数としてこう書きます。
t:{a},{b},{n}
BPMはこのようにします
b:<BPM>
...
コードの場合、+記号を使って複数の音をつなげます。パーカッションはGeneralMIDIに準拠したノートで、他のパートと同じようにC2などと書いてください。
[強さ]a-g aが最弱で、g だと最大の強さになります。内部はMidiのvelocityなので、gだと127になります。

また、オクターブと長さと強さはそれぞれ直前と同じ場合は省略できます。
なるべく省略形で書いてください。

省略前 M:C4qgD4qgE4qdC4qd
省略後 M:C4qgDEdC

また、シーケンサーなので音を鳴らさない箇所には必ず休符をつけてください。
シーケンスは必ずコードブロックに入れてください。
トークン数節約のために、上記のようになるべく省略形で書くことを心がけてください。)";
    }
};
